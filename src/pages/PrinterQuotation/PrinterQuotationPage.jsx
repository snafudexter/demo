...
const ADD_PRINTER_QUOTATION = gql`
  mutation addPrinterQuotation(
    $printerId: ID!
    $cityId: ID!
    $mediaIdArray: [ID!]
...
`;
class PrinterQuotationPage extends React.Component {
  state = {
    selectedCityId: "",
    selectedCity: "",
    mediaCostArray: [], // contains obj with format {id,name,cost}
    transportCost: -1,
    installationCost: -1,
    recceRate: -1,
    recceRatePerShop: true,
    hubRecceRate: -1,
    hubRecceRatePerShop: true,
    showHubRates: false,
    hubTransportationCost: -1,
    hubInstallationCost: -1
  };
  ...
  renderHubRecceRateAndPerType = () => {
    console.log(this.state.hubRecceRatePerShop);
    return (
      <div>
        <div>
          <div className="text-center">
  ...
  renderAddPrinterQuotationMutation = () => {
    return (
      <Mutation mutation={ADD_PRINTER_QUOTATION}>
        {(addPrinterQuotation, { data }) => (
          <div className="modal-footer">
            <button
              ...
              onClick={e => {
                ...
                if (
                  this.props.printerName.length <= 0 ||
                  this.props.printerId.length <= 0
                ) {
                  Alert.closeAll();
                ...
                addPrinterQuotation({
                  ...
                  .then(resp => {
                    // console.log(resp);
                    Alert.closeAll();
                    Alert.success(`Done!! Printer Quotation Added`, {
                      position: "top-right",
                      effect: "slide"
                    });
                  })
              ...
              <div>
                <div className="row">
                  ...
                  >
                    ...
                    <div className="form-group">
                      <div>{this.renderAddPrinterQuotationMutation()}</div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div id="page-header" className="bg-gray-lighter">
            <div className="content p-0">
              <div className="content-header">
                <strong>Add New Printer Quotation </strong>
              </div>
            </div>
          </div>
        </div>
        <div id="main-container">
          <div>{this.renderAddPrinterQuotationForm()}</div>
        </div>
      </div>
    );
  };
...
export default PrinterQuotationPage;
